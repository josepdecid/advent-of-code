...
fn is_equation_solvable(result: &u64, terms: &Vec<u64>) -> bool {
    ...
    let total_combinations = 3_u32.pow(number_of_operations);

    for i in 0..total_combinations {
        let mut combination_result = terms[0];
        let combination = (0..number_of_operations)
            .map(|j| ((i / 3_u32.pow(j)) % 3) as u8)
            .collect::<Vec<u8>>();
        ...
        for j in 1..terms.len() {
            if combination[j - 1] == 0 {
                ...
                let second_digits = terms[j].to_string().len() as u32;
                combination_result = combination_result * 10_u64.pow(second_digits) + terms[j];
            }
        }
...
fn main() {
    ...
    println!("Result: {}", count);  // 162987117690649   
}
